...
Use a Usage Budget Checkpoint Gate for long or hard-to-interrupt work. Codex cannot directly inspect the Codex 5h usage meter; use `dev_locals/work-log.md` elapsed work time as a proxy. Around 90 minutes of continuous work, remind the user to check remaining usage. Around 2 hours, before starting long analysis, complex planning, broad refactors, publish flow, browser verification, CI debug, rebase/merge, or similar work, ask the user to check usage and choose whether to continue, narrow scope, checkpoint then pause, or proceed in smaller chunks. Do not automatically switch to conservative mode; if the user explicitly continues without usage information, continue in smaller coherent slices with more frequent checkpoints.
